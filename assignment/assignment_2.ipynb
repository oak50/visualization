...
sns.kdeplot(data=df,x='Telehealth',hue='Age')
plt.show()
plt.savefig("kde_telehealth_age")

# -------------

#6. Create a scatter plot, and describe the relationship you see.

sns.scatterplot(data=df, x="Age", y="Telehealth")
plt.show()
plt.savefig("age_telehealth_scatter.png")

#7. Create a table (cross tabulation, variance-covariance, or a .describe() five-number summary), and explain what you see.

print(pd.crosstab(df['Age'],df['Telehealth']),'\n')
print(pd.crosstab(df['Age'],df['Telehealth'], normalize = 'all'),'\n')
